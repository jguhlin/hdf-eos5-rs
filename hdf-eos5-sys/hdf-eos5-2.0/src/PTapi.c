...
HE5_PTattach(hid_t fid, const char *pointname)
{
  ...
  if (status == SUCCEED)
	...
	npointopen += HE5_PTXPoint[i].active;
	  ...
	  idx = H5Giterate(data_id, grpname , NULL, (H5G_iterate_t)HE5_PTopenlevel, &tmpdata);
	  if ( idx == FAIL )
	    {
	  ...
	  if (info->dtype[i] == FAIL)
	    {
	      ...
	      HE5_EHprint(errbuf, __FILE__, __LINE__);
	      if (info->fieldname[i] != NULL)
		    H5free_memory(info->fieldname[i]);
	      if (info != NULL) free(info);
	      return(FAIL);
	    }
...
HE5_PTdeflevel(hid_t  pointID, const char *levelname, HE5_CmpDTSinfo *levelinfo)
{
  ...
  /* Create (appendable) dataset for the level */
  /* ----------------------------------------- */
  ...
  if (nlevels > 0)
      ...
      if (status == FAIL )
	{
      ...
      /* Create (appendable) compound dataset */
      /* ------------------------------------ */
...
HE5_PTwritelevel(hid_t pointID, int level, hsize_t count[], size_t *size, void *data)
{
  ...
  if(nrec > 0)
      ...
      if (readstatus == FAIL )
	{
      ...
      val = (unsigned char *)calloc(nrec, datasize);
      if(val == NULL)
	{
      ...
      if (readstatus == FAIL )
	{
      ...
      newdatabuff = (unsigned char*)calloc((nrec+tcount[0]), datasize);
  ...
  for (i = 0; i < dtsinfo.nfields; i++)
    {
      if (dtsinfo.fieldname[i] != NULL)
	    H5free_memory(dtsinfo.fieldname[i]);
    }
...
-----------------------------------------------------------------------------*/
herr_t
HE5_PTattrinfo2(hid_t pointID, const char *attrname, hid_t *ntype, hsize_t *count, hsize_t *size)
{
  herr_t        status     = FAIL;/* routine return status variable */
  ...
  /* Check for valid point ID */
  /* ------------------------ */
  status = HE5_PTchkptid(pointID, "HE5_PTattrinfo2", &fid, &gid, &idx);
  if (status == SUCCEED)
    {
      /* Get attribute group ID and call EHattrinfo */
      status = HE5_EHattrinfo2( HE5_PTXPoint[idx].pt_id, attrname, ntype, count, size);
    }
...
-----------------------------------------------------------------------------*/
herr_t
HE5_PTgrpattrinfo2(hid_t pointID, const char *attrname, hid_t *ntype, hsize_t *count, hsize_t *size)
{
  herr_t     status     = FAIL;/* routine return status variable */
  ...
  /* Check for valid point ID */
  /* ------------------------ */
  status = HE5_PTchkptid(pointID, "HE5_PTgrpattrinfo2", &fid, &gid, &idx);
  if (status == SUCCEED)
    {
      /* Get "Data"  group ID  */
      /* --------------------- */
	...
	{
	  sprintf(errbuf,"Cannot open the \"Data\" group ID.\n");
	  H5Epush(__FILE__, "HE5_PTgrpattrinfo2", __LINE__, H5E_OHDR, H5E_NOTFOUND, errbuf);
	  HE5_EHprint(errbuf, __FILE__, __LINE__);
	  return(FAIL);
	}
      ...
      /* Call HE5_EHattrinfo */
      /* ------------------- */
      status = HE5_EHattrinfo2(DataGrpID, attrname, ntype, count, size);
      if(status == FAIL)
	{
	  sprintf(errbuf,"Cannot retrieve information about Attribute \"%s\" in the \"Data\" group.\n", attrname);
	  H5Epush(__FILE__, "HE5_PTgrpattrinfo2", __LINE__, H5E_ATTR, H5E_NOTFOUND, errbuf);
	  HE5_EHprint(errbuf, __FILE__, __LINE__);
	  return(status);
	}
	...
	{
	  sprintf(errbuf,"Cannot release the \"Data\" group ID.\n");
	  H5Epush(__FILE__, "HE5_PTgrpattrinfo2", __LINE__, H5E_OHDR, H5E_CLOSEERROR, errbuf);
	  HE5_EHprint(errbuf, __FILE__, __LINE__);
	  return(status);
	}
    }
...
-----------------------------------------------------------------------------*/
herr_t
HE5_PTlocattrinfo2(hid_t pointID, const char *levelname, const char *attrname, hid_t *ntype, hsize_t *count, hsize_t *size)
{
  herr_t     status     = FAIL;/* routine return status variable */
  ...
  /* Check for valid point ID */
  /* ------------------------ */
  status = HE5_PTchkptid(pointID, "HE5_PTlocattrinfo2", &fid, &gid, &idx);
  if (status == SUCCEED)
    {
      /* Get "Data"  group ID  */
      /* --------------------- */
	...
	{
	  sprintf(errbuf,"Cannot open the \"Data\" group ID.\n");
	  H5Epush(__FILE__, "HE5_PTlocattrinfo2", __LINE__, H5E_OHDR, H5E_NOTFOUND, errbuf);
	  HE5_EHprint(errbuf, __FILE__, __LINE__);
	  return(FAIL);
	}
      /* Get level ID  */
	...
	{
	  sprintf(errbuf,"Cannot open the \"%s\" level dataset ID.\n", levelname);
	  H5Epush(__FILE__, "HE5_PTlocattrinfo2", __LINE__, H5E_DATASET, H5E_NOTFOUND, errbuf);
	  HE5_EHprint(errbuf, __FILE__, __LINE__);
	  return(FAIL);
	}
      ...
      /* Call EHattrinfo */
      /* --------------- */
      status = HE5_EHattrinfo2(levelID, attrname, ntype, count, size);
      if(status == FAIL)
	{
	  sprintf(errbuf,"Cannot retrieve information about Attribute \"%s\" in the \"%s\" level.\n", attrname,levelname);
	  H5Epush(__FILE__, "HE5_PTlocattrinfo2", __LINE__, H5E_ATTR, H5E_NOTFOUND, errbuf);
	  HE5_EHprint(errbuf, __FILE__, __LINE__);
	  return(status);
	}
	...
	{
	  sprintf(errbuf,"Cannot release the \"%s\" level dataset ID.\n", levelname);
	  H5Epush(__FILE__, "HE5_PTlocattrinfo2", __LINE__, H5E_DATASET, H5E_CLOSEERROR, errbuf);
	  HE5_EHprint(errbuf, __FILE__, __LINE__);
	  return(status);
        }
	...
	{
	  sprintf(errbuf,"Cannot release the \"Data\" group ID.\n");
	  H5Epush(__FILE__, "HE5_PTlocattrinfo2", __LINE__, H5E_OHDR, H5E_CLOSEERROR, errbuf);
	  HE5_EHprint(errbuf, __FILE__, __LINE__);
	  return(status);
	}
    }
  ...
  for (k = 0; k < currLevel.nfields; k++)
      ...
      if (memcmp(currLevel.fieldname[k], utlbuf, strlen(utlbuf)) == 0)
	{
      ...
      if (currLevel.fieldname[k] != NULL)
	    H5free_memory(currLevel.fieldname[k]);
    }
  ...
  if( nrec == 0 || recs == NULL )
      ...
      for (j = 0; j < info.nfields; j++)
	{
	  if (info.fieldname[j] != NULL)
	    H5free_memory(info.fieldname[j]);
	}
  ...
  for (j = 0; j < infolevel.nfields; j++)
    {
      if (infolevel.fieldname[j] != NULL)
	    H5free_memory(infolevel.fieldname[j]);
    }
...
HE5_PTupdatelevel_f(hid_t pointID, int level, char *fieldlist, hsize_t nrec,  hssize_t recs[], int ntype, hid_t dtype, void *data)
{
  ...
  for (i = 0; i < nfields; i++)
      ...
      if (HE5_PTfexist(dataID, utlbuf, NULL, NULL, NULL, NULL) != 1)
	{
	  ...
	  HE5_EHprint(errbuf, __FILE__, __LINE__);
	  if (fieldname != NULL) H5free_memory(fieldname);
	  return(status);
	}
    }
  ...
  for (j = 0; j < dtsinfo.nfields; j++)
    {
      if (dtsinfo.fieldname[j] != NULL)
	    H5free_memory(dtsinfo.fieldname[j]);
    }
...
|  BEGIN_PROLOG                                                               |
|                                                                             |
|  FUNCTION: HE5_PTattrinfoF2   (FORTRAN wrapper)                             |
|                                                                             |
|  DESCRIPTION:                                                               |
|                                                                             |
|                                                                             |
...
|  BEGIN_PROLOG                                                               |
|                                                                             |
|  FUNCTION: HE5_PTgrpattrinfoF2   (FORTRAN wrapper)                          |
|                                                                             |
|  DESCRIPTION:  Retrieves information about attributes in "Data" group.      |
|                                                                             |
|                                                                             |
...
HE5_PTgrpattrinfoF2(int pointID, const char *attrname, int *numbertype, long *fortcount, long *fortsize)
{
  ...
  if(errbuf == NULL)
    {
      H5Epush(__FILE__, "HE5_PTgrpattrinfoF2", __LINE__, H5E_RESOURCE, H5E_NOSPACE, "Cannot allocate memory for error buffer.");
      HE5_EHprint("Error: Cannot allocate memory for error buffer, occured", __FILE__, __LINE__);
      return(FAIL);
    }
  ...
  if( count == NULL)
    {
      sprintf(errbuf,"Cannot allocate memory for count.\n");
      H5Epush(__FILE__, "HE5_PTgrpattrinfoF2", __LINE__, H5E_RESOURCE, H5E_NOSPACE, errbuf);
      HE5_EHprint(errbuf, __FILE__, __LINE__);
      free(errbuf);
      return(FAIL);
    }
...
|  BEGIN_PROLOG                                                               |
|                                                                             |
|  FUNCTION: HE5_PTlocattrinfoF2   (FORTRAN wrapper)                          |
|                                                                             |
|  DESCRIPTION:  Retrieves information about attributes in associated with a  |
|                 specified level in a point.                                 |
|                                                                             |
...
HE5_PTlocattrinfoF2(int pointID, const char *levelname, const char *attrname, int *numbertype, long *fortcount, long *fortsize)
{
  ...
  if(errbuf == NULL)
    {
      H5Epush(__FILE__, "HE5_PTlocattrinfoF2", __LINE__, H5E_RESOURCE, H5E_NOSPACE, "Cannot allocate memory for error buffer.");
      HE5_EHprint("Error: Cannot allocate memory for error buffer, occured", __FILE__, __LINE__);
      return(FAIL);
    }
  ...
  if( count == NULL)
    {
      sprintf(errbuf,"Cannot allocate memory for count.\n");
      H5Epush(__FILE__, "HE5_PTlocattrinfoF2", __LINE__, H5E_RESOURCE, H5E_NOSPACE, errbuf);
      HE5_EHprint(errbuf, __FILE__, __LINE__);
      free(errbuf);
      return(FAIL);
    }
...
FCALLSCFUN5(INT, HE5_PTattrinfoF2,HE5_PTATTRINFO2, he5_ptattrinfo2, INT, PSTRING, PINT, PLONG, PLONG)
FCALLSCFUN5(INT, HE5_PTgrpattrinfoF2,HE5_PTGRPATTRINFO2, he5_ptgrpattrinfo2, INT, PSTRING, PINT, PLONG, PLONG)
FCALLSCFUN6(INT, HE5_PTlocattrinfoF2,HE5_PTLOCATTRINFO2, he5_ptlocattrinfo2, INT, PSTRING, PSTRING, PINT, PLONG, PLONG)
