...
int HE5_ZAsetflddimlabel(hid_t zaID, char *fieldname, char *dimname, char *label);
int HE5_ZAgetflddimlabel(hid_t zaID, char *fieldname, char *dimname, char *label);
herr_t HE5_ZAsetdimscale2(hid_t zaID, char *fieldname, char *dimname, char *dimscalename, const hsize_t dimsize, hid_t numbertype_in, void * data);
...
int
HE5_ZAatinfo2(int zaID, char *attrname, int *ntype, long *fortcount, long *fortsize);
int
HE5_ZAgatinfo2(int zaID, char *attrname, int *ntype, long *fortcount, long *fortsize);
int
HE5_ZAlatinfo2(int zaID, char *fieldname, char *attrname, int *ntype, long *fortcount, long *fortsize);
long
...
|  BEGIN_PROLOG                                                               |
|                                                                             |
|  FUNCTION: HE5_ZAdefdimscale2                                               |
|                                                                             |
|  DESCRIPTION: Defines dimension scale for all fields that use the given     |
|               dimesion (user defined dimension scale name)                  |
|                                                                             |
|                                                                             |
|  Return Value    Type     Units     Description                             |
|  ============   ======  =========   =====================================   |
...
|                                                                             |
|  NOTES: This function finds out which field in a ZA has the dimension       |
|         and sets dimension scale for that field calling HE5_ZAsetdimscale2()|
|         User must detach ZA after creating fields and attach again before   |
|         calling the routine.                                                |
|                                                                             |
|                                                                             |
...
-----------------------------------------------------------------------------*/
herr_t
HE5_ZAdefdimscale2(hid_t zaID,  char *dimname, char *dimscalename,
		  const hsize_t dimsize, hid_t numbertype_in, void * data)
{
  herr_t       status    = FAIL;/* routine return status variable  */
  hid_t        fid       = FAIL;/* HDF-EOS file ID                 */
  ...
  /* Get HDF-EOS file ID, "HDFEOS" group ID and Za index */
  /* ------------------------------------------------------ */
  status = HE5_ZAchkzaid(zaID, "HE5_ZAdefdimscale2", &fid, &gid, &idx);
  if ( status == FAIL )
    {
      sprintf(errbuf, "Checking for Za ID failed.\n");
      H5Epush(__FILE__, "HE5_ZAdefdimscale2", __LINE__, H5E_ARGS, H5E_BADRANGE, errbuf);
      HE5_EHprint(errbuf, __FILE__, __LINE__);
      return(status);
    }
  ...
  if ( dimsize == 0 )
    {
      status = FAIL;
      sprintf(errbuf, "Invalid (zero) dimension size.\n");
      H5Epush(__FILE__, "HE5_ZAdefdimscale2", __LINE__, H5E_ARGS, H5E_BADVALUE, errbuf);
      HE5_EHprint(errbuf, __FILE__, __LINE__);
      return(status);
    }
  ...
  */
  ...
  if (nflds == FAIL)
    {
      sprintf(errbuf, "Cannot get the number of fields in \"Data Fields\" group. \n");
      H5Epush(__FILE__, "HE5_ZAdefdimscale2", __LINE__,
	      H5E_ARGS, H5E_BADVALUE, errbuf);
      HE5_EHprint(errbuf, __FILE__, __LINE__);
      return(FAIL);
    }
	  ...
	  if(ntflds != FAIL)
	      ...
	      for(i = 0; i < ntflds; i++)
		{
		  ...
		  if (status != SUCCEED)
		    {
		      sprintf(errbuf, "Field \"%s\" not found.\n", tempfield);
		      H5Epush(__FILE__, "HE5_ZAdefdimscale2", __LINE__, H5E_ARGS,
			      H5E_NOTFOUND, errbuf);
		      HE5_EHprint(errbuf, __FILE__, __LINE__);
		      free(tempfield);
		      free(fieldlist);
		  ...
		  if (Dimindex == FAIL) /* dimension not found */
		    ...
		    {
		      /* call HE5_ZAsetdimscle */
		      status = HE5_ZAsetdimscale2(zaID, (char *)tempfield,
	...
	if ((H5DSis_scale(dsid)) <= 0 )
	  {
	    ...
	    if(HE5_ZAsetflddimlabel(zaID, fieldname, dimname, dimscalename) < 0)
	      {
		status = 0;
		  ...
		  const hsize_t dimsize, hid_t numbertype_in, void * data)
{
  ...
  if(errbuf == NULL)
    {
      H5Epush(__FILE__, "HE5_ZAsetdimscale2", __LINE__, H5E_FILE, H5E_NOSPACE, "Cannot allocate memory for error buffer.");
      HE5_EHprint("Error: Cannot allocate memory for error buffer ", __FILE__, __LINE__);
      return(FAIL);
    }
  ...
  if(ntype == FAIL)
    {
      sprintf(errbuf,"Cannot convert to HDF5 type data type ID for dimscale\n");
      H5Epush(__FILE__, "HE5_ZAsetdimscale2", __LINE__, H5E_DATATYPE, H5E_BADVALUE, errbuf);
      HE5_EHprint(errbuf, __FILE__, __LINE__);
      return(FAIL);
    }
...
**********************************************************
*/
  status = HE5_ZAchkzaid(zaID, "HE5_ZAsetdimscale2", &fid, &gid, &idx);
  if(status == FAIL)
    {
      sprintf(errbuf,"Checking for ZA ID failed.\n");
      H5Epush(__FILE__, "HE5_ZAsetdimscale2", __LINE__, H5E_FUNC, H5E_CANTINIT, errbuf);
      HE5_EHprint(errbuf, __FILE__, __LINE__);
      free(errbuf);
      return(FAIL);
    }
  ...
  if (status == 0)
	...
	{
	  sprintf(errbuf, "Fieldname \"%s\" does not exist.\n", fieldname);
	  H5Epush(__FILE__, "HE5_ZAsetdimscale2", __LINE__,  H5E_ARGS, H5E_BADRANGE, errbuf);
	  HE5_EHprint(errbuf, __FILE__, __LINE__);
	  free(errbuf);
	  return(FAIL);
	}
	...
	if (Dimindex == FAIL) /* dimension not found */
	  {
	    ...
	    sprintf( errbuf, "Dimname \"%s\" does not exist for field \"%s\".\n",
		     dimname, fieldname );
	    H5Epush(__FILE__, "HE5_ZAsetdimscale2", __LINE__, H5E_IO, H5E_SEEKERROR, errbuf);
	    HE5_EHprint(errbuf, __FILE__, __LINE__);
	    free(errbuf);
	    return(status);
	  }
	...
	dsid = H5Dopen(HE5_ZAXZa[idx].za_id, dimscalename);
	if(dsid < 0)
	  {
	    /* create Dim Scale dataset */
	    /* Try to open the " group */
	    ...
	    dsid = H5Dcreate(HE5_ZAXZa[idx].za_id,dimscalename,ntype, data_space, dsplist);
	    ...
	    if ( fspace == FAIL)
	      {
		sprintf(errbuf, "Cannot get the file data space ID.\n");
		H5Epush(__FILE__, "HE5_ZAsetdimscale2", __LINE__, H5E_DATASPACE, H5E_NOTFOUND, errbuf);
		HE5_EHprint(errbuf, __FILE__, __LINE__);
		free(errbuf);
		return(FAIL);
	      }
	    ...
	    if( status == FAIL )
	      {
		sprintf(errbuf,"Cannot write data to the dataset.\n");
		H5Epush(__FILE__, "HE5_ZAsetdimscale2", __LINE__, H5E_DATASET, H5E_WRITEERROR, errbuf);
		HE5_EHprint(errbuf, __FILE__, __LINE__);
		free(errbuf);
		return(FAIL);
	      }
...
|  BEGIN_PROLOG                                                               |
|                                                                             |
|  FUNCTION: HE5_ZAdscaleattrinfo2                                            |
|                                                                             |
|  DESCRIPTION: Retrieves information about dimension scale attribute         |
|               (attribute associated with a specified dimension scale field) |
|                in a ZA.                                                     |
...
-----------------------------------------------------------------------------*/
herr_t
HE5_ZAdscaleattrinfo2(hid_t zaID, const char *fieldname, const char *attrname, hid_t *ntype, hsize_t *count, hsize_t *size)
{
  herr_t     status        = FAIL;/* routine return status variable */
...
*/
  ...
  if (status == SUCCEED)
	...
	{
	  sprintf(errbuf,"Cannot open the \"%s\" field dataset ID.\n", fieldname);
	  H5Epush(__FILE__, "HE5_ZAdscaleattrinfo2", __LINE__, H5E_DATASET, H5E_NOTFOUND, errbuf);
	  HE5_EHprint(errbuf, __FILE__, __LINE__);
	  return(FAIL);
	}
						  ...
						  scale attribute */
	{
	  ...
	  if(status == FAIL)
	    {
	      sprintf(errbuf,"Cannot retrieve information about Attribute \"%s\" associated with the \"%s\" field.\n", attrname, fieldname);
	      H5Epush(__FILE__, "HE5_ZAdscaleattrinfo2", __LINE__, H5E_FUNC, H5E_CANTINIT, errbuf);
	      HE5_EHprint(errbuf, __FILE__, __LINE__);
	      return(status);
	    }
	}
	...
	{
	  sprintf(errbuf,"Cannot release the \"%s\" field dataset ID.\n", fieldname);
	  H5Epush(__FILE__, "HE5_ZAdscaleattrinfo2", __LINE__, H5E_DATASET, H5E_CLOSEERROR, errbuf);
	  HE5_EHprint(errbuf, __FILE__, __LINE__);
	  return(status);
	}
    }
...
*/
  ...
  if (numbertype != HE5T_CHARSTRING)
    {
      if (H5Tequal(numbertype, H5T_NATIVE_CHAR)   == FALSE &&
...
-----------------------------------------------------------------------------*/
herr_t
HE5_ZAattrinfo2(hid_t zaID, const char *attrname, hid_t *ntype, hsize_t *count, hsize_t *size)
{
  herr_t        status  = FAIL;             /* Return status variable  */
  ...
  /* Get HDF-EOS file ID, "HDFEOS" group ID and za index */
  /* --------------------------------------------------- */
  status = HE5_ZAchkzaid(zaID, "HE5_ZAattrinfo2", &fid, &gid, &idx);
  if (status == SUCCEED)
    {
      /* Get attribute group ID and call HE5_EHattrinfo */
      /* ---------------------------------------------- */
      status = HE5_EHattrinfo2(HE5_ZAXZa[idx].za_id, attrname, ntype, count, size);
      if ( status == FAIL )
	{
	  sprintf(errbuf, "Cannot retrieve information about attribute. \n");
	  H5Epush(__FILE__, "HE5_ZAattrinfo2", __LINE__, H5E_ATTR, H5E_NOTFOUND, errbuf);
	  HE5_EHprint(errbuf, __FILE__, __LINE__);
	}
    }
...
-----------------------------------------------------------------------------*/
herr_t
HE5_ZAgrpattrinfo2(hid_t zaID, const char *attrname, hid_t *ntype, hsize_t *count, hsize_t *size)
{
  herr_t     status          = FAIL;     /* routine return status variable */
  ...
  /* Get HDF-EOS file ID, "HDFEOS" group ID and za index */
  /* --------------------------------------------------- */
  status = HE5_ZAchkzaid(zaID, "HE5_ZAgrpattrinfo2", &fid, &gid, &idx);
  if (status == SUCCEED)
    {
      /* Get "Data Fields"  group ID and call HE5_EHattrinfo */
      /* --------------------------------------------------- */
	...
	{
	  sprintf(errbuf,"Cannot open the \"Data Fields\" group ID.\n");
	  H5Epush(__FILE__, "HE5_ZAgrpattrinfo2", __LINE__, H5E_OHDR, H5E_NOTFOUND, errbuf);
	  HE5_EHprint(errbuf, __FILE__, __LINE__);
	  return(FAIL);
	}
	...
	{
	  sprintf(errbuf,"Cannot retrieve information about Attribute \"%s\" in the \"Data Fields\" group.\n", attrname);
	  H5Epush(__FILE__, "HE5_ZAgrpattrinfo2", __LINE__, H5E_FUNC, H5E_CANTINIT, errbuf);
	  HE5_EHprint(errbuf, __FILE__, __LINE__);
	  return(status);
	}
	...
	{
	  sprintf(errbuf,"Cannot release the \"Data Fields\" group ID.\n");
	  H5Epush(__FILE__, "HE5_ZAgrpattrinfo2", __LINE__, H5E_OHDR, H5E_CLOSEERROR, errbuf);
	  HE5_EHprint(errbuf, __FILE__, __LINE__);
	  return(status);
	}
...
-----------------------------------------------------------------------------*/
herr_t
HE5_ZAlocattrinfo2(hid_t zaID, const char *fieldname, const char *attrname, hid_t *ntype, hsize_t *count, hsize_t *size)
{
  herr_t     status          =  FAIL;    /* Return status variable */
  ...
  /* Get HDF-EOS file ID, "HDFEOS" group ID and za index */
  /* --------------------------------------------------- */
  status = HE5_ZAchkzaid(zaID, "HE5_ZAlocattrinfo2", &fid, &gid, &idx);
  if (status == SUCCEED)
    {
      /* Allocate memory for the group name buffer */
      /* ----------------------------------------- */
      ...
      if( grpname == NULL)
        {
	  sprintf(errbuf,"Cannot allocate memory for the group name buffer.\n");
          H5Epush(__FILE__, "HE5_ZAlocattrinfo2", __LINE__, H5E_RESOURCE, H5E_NOSPACE, errbuf);
          HE5_EHprint(errbuf, __FILE__, __LINE__);
          return(FAIL);
        }
	...
	{
	  sprintf(errbuf,"Cannot get the group flag for \"%s\" field. \n",fieldname);
	  H5Epush(__FILE__, "HE5_ZAlocattrinfo2", __LINE__, H5E_OHDR, H5E_NOTFOUND, errbuf);
	  HE5_EHprint(errbuf, __FILE__, __LINE__);
	  free(grpname);
	  return(FAIL);
	}
	...
	{
	  sprintf(errbuf,"Cannot open the \"%s\" field dataset.\n", fieldname);
	  H5Epush(__FILE__, "HE5_ZAlocattrinfo2", __LINE__, H5E_DATASET, H5E_NOTFOUND, errbuf);
	  HE5_EHprint(errbuf, __FILE__, __LINE__);
	  free(grpname);
	  return(FAIL);
	}
	...
	{
	  sprintf(errbuf,"Cannot retrieve information about Attribute \"%s\" in the \"%s\" group.\n", attrname, grpname);
	  H5Epush(__FILE__, "HE5_ZAlocattrinfo2", __LINE__, H5E_ATTR, H5E_NOTFOUND, errbuf);
	  HE5_EHprint(errbuf, __FILE__, __LINE__);
	  free(grpname);
	  return(status);
	}
	...
	{
	  sprintf(errbuf,"Cannot release the \"%s\" field dataset ID.\n", fieldname);
	  H5Epush(__FILE__, "HE5_ZAlocattrinfo2", __LINE__, H5E_DATASET, H5E_CLOSEERROR, errbuf);
	  HE5_EHprint(errbuf, __FILE__, __LINE__);
	  free(grpname);
	  return(status);
	}
	...
	{
	  sprintf(errbuf,"Cannot release the field group ID.\n");
	  H5Epush(__FILE__, "HE5_ZAlocattrinfo2", __LINE__, H5E_OHDR, H5E_CLOSEERROR, errbuf);
	  HE5_EHprint(errbuf, __FILE__, __LINE__);
	  free(grpname);
	  return(status);
	}
...
|  BEGIN_PROLOG                                                               |
|                                                                             |
|  FUNCTION: HE5_ZAatinfo2                                                    |
|                                                                             |
|  DESCRIPTION:  FORTRAN wrapper for ZAattrinfo2                              |
|                                                                             |
|                                                                             |
|  Return Value    Type     Units     Description                             |
|  ============   ======  =========   =====================================   |
...
-----------------------------------------------------------------------------*/
int
HE5_ZAatinfo2(int ZaID, char *attrname, int *numbertype, long *fortcount, long *fortsize)
{
  int              ret      = FAIL;        /* (int) return status variable   */
  ...
  hsize_t          *count   = (hsize_t *)NULL;/* Pointer to count            */

  hsize_t          *size    = (hsize_t *)NULL;/* Pointer to array elemet size*/
  ...
  if(errbuf == NULL)
    {
      H5Epush(__FILE__, "HE5_ZAatinfo2", __LINE__, H5E_RESOURCE, H5E_NOSPACE, "Cannot allocate memory for error buffer.");
      HE5_EHprint("Error: Cannot allocate memory for error buffer, occured", __FILE__, __LINE__);
      return(FAIL);
    }
  ...
  if( count == NULL)
    {
      sprintf(errbuf,"Cannot allocate memory for count.\n");
      H5Epush(__FILE__, "HE5_ZAatinfo2", __LINE__, H5E_RESOURCE, H5E_NOSPACE, errbuf);
      HE5_EHprint(errbuf, __FILE__, __LINE__);
      free(errbuf);
      return(FAIL);
    }
  ...
  status = HE5_ZAattrinfo2(zaID, attrname, ntype, count, size);
  if(status == FAIL)
    {
      sprintf(errbuf,"Cannot get information about attribute \"%s\".\n", attrname);
      H5Epush(__FILE__, "HE5_ZAatinfo2", __LINE__, H5E_ATTR, H5E_NOTFOUND, errbuf);
      HE5_EHprint(errbuf, __FILE__, __LINE__);
      free(errbuf);
      free(count);
      free(ntype);
...
|  BEGIN_PROLOG                                                               |
|                                                                             |
|  FUNCTION: HE5_ZAgatinfo2                                                   |
|                                                                             |
|  DESCRIPTION:  FORTRAN wrapper for ZAgrpattrinfo2                           |
|                                                                             |
|                                                                             |
|  Return Value    Type     Units     Description                             |
|  ============   ======  =========   =====================================   |
...
-----------------------------------------------------------------------------*/
int
HE5_ZAgatinfo2(int ZaID, char *attrname, int *numbertype, long *fortcount, long *fortsize)
{
  int	   	  ret     = FAIL;	    /* (int) return status variable   */
  ...
  hsize_t     *size   = (hsize_t *)NULL;/* Pointer to size of array element*/
  ...
  if(errbuf == NULL)
    {
      H5Epush(__FILE__, "HE5_ZAgatinfo2", __LINE__, H5E_RESOURCE, H5E_NOSPACE, "Cannot allocate memory for error buffer.");
      HE5_EHprint("Error: Cannot allocate memory for error buffer, occured", __FILE__, __LINE__);
      return(FAIL);
    }
  ...
  if( count == NULL)
    {
      sprintf(errbuf,"Cannot allocate memory for count.\n");
      H5Epush(__FILE__, "HE5_ZAgatinfo2", __LINE__, H5E_RESOURCE, H5E_NOSPACE, errbuf);
      HE5_EHprint(errbuf, __FILE__, __LINE__);
      free(errbuf);
      return(FAIL);
    }
  ...
  status = HE5_ZAgrpattrinfo2(zaID, attrname, ntype, count, size);
  if(status == FAIL)
    {
      sprintf(errbuf,"Cannot get information about group attribute.\n");
      H5Epush(__FILE__, "HE5_ZAgatinfo2", __LINE__, H5E_RESOURCE, H5E_NOSPACE, errbuf);
...
|  BEGIN_PROLOG                                                               |
|                                                                             |
|  FUNCTION: HE5_ZAlatinfo2                                                   |
|                                                                             |
|  DESCRIPTION:  FORTRAN wrapper for ZAlocattrinfo2                           |
|                                                                             |
|                                                                             |
|  Return Value    Type     Units     Description                             |
|  ============   ======  =========   =====================================   |
...
-----------------------------------------------------------------------------*/
int
HE5_ZAlatinfo2(int ZaID, char *fieldname, char *attrname, int *numbertype, long *fortcount, long *fortsize)
{
  int	   	  ret     = FAIL;	        /* (int) return status variable   */
  ...
  if(errbuf == NULL)
    {
      H5Epush(__FILE__, "HE5_ZAlatinfo2", __LINE__, H5E_RESOURCE, H5E_NOSPACE, "Cannot allocate memory for error buffer.");
      HE5_EHprint("Error: Cannot allocate memory for error buffer, occured", __FILE__, __LINE__);
      return(FAIL);
    }
...
|  BEGIN_PROLOG                                                               |
|                                                                             |
|  FUNCTION: HE5_ZAsetdimscale2F (FORTRAN wrapper for HE5_ZAsetdimscale2)     |
|                                                                             |
|  DESCRIPTION: Defines a dimension scale for a fields dimension in a ZA      |
|                                                                             |
|                                                                             |
...
-----------------------------------------------------------------------------*/
int
HE5_ZAsetdimscale2F(int ZaID, char *fieldname, char *dimname, char *dimscalename,
		  long dim, int numbertype_in, void * data)
{
  ...
  status = HE5_ZAsetdimscale2(zaID, fieldname, dimname, dimscalename,
			     (hsize_t) dim, (hid_t) numbertype_in, data);

  if (status < 0)
    {
      sprintf(errbuf, "Cannot set dimension scale %s for field %s. \n", dimname, fieldname);
      H5Epush(__FILE__, "HE5_ZAsetdimscale2F", __LINE__, H5E_DATASET, H5E_WRITEERROR, errbuf);
      HE5_EHprint(errbuf, __FILE__, __LINE__);
    }
...
|  BEGIN_PROLOG                                                               |
|                                                                             |
|  FUNCTION: HE5_ZAdefdimscale2F (FORTRAN wrapper for HE5_ZAdefdimscale2)     |
|                                                                             |
...
-----------------------------------------------------------------------------*/
int
HE5_ZAdefdimscale2F(int ZaID, char *dimname, char *dimscalename,
		  long dim, int numbertype_in, void * data)
{
  herr_t          status;
  hid_t           zaID;
  ...
  status = HE5_ZAdefdimscale2(zaID, dimname, dimscalename,
			     (hsize_t) dim, (hid_t) numbertype_in, data);

  if (status < 0)
    {
      sprintf(errbuf, "Cannot set dimension scale %s for all fields in ZA. \n", dimname);
      H5Epush(__FILE__, "HE5_ZAdefdimscale2F", __LINE__, H5E_DATASET, H5E_WRITEERROR, errbuf);
      HE5_EHprint(errbuf, __FILE__, __LINE__);
    }
...
|  BEGIN_PROLOG                                                               |
|                                                                             |
|  FUNCTION: HE5_ZAdscaleattrinfoF2 (FORTRAN wrapper)                          |
|                                                                             |
|  DESCRIPTION: Retrieves information about dimension scale attribute         |
|               (attribute associated with a specified dimension scale field) |
|                in a ZA.                                                     |
...
-----------------------------------------------------------------------------*/
int
HE5_ZAdscaleattrinfoF2(int ZaID, char *fieldname, char *attrname, int *ntype, long *fortcount, long *fortsize)
{
  herr_t          status         = FAIL;       /* return status             */
  hid_t           zaID           = FAIL;       /* HDF5 type za ID           */
  char            errbuf[HE5_HDFE_ERRBUFSIZE]; /* Buffer for error message  */
  hsize_t         *count = (hsize_t *)NULL;    /* count of array elements   */
  ...
  if(dtype == (hid_t *)NULL)
    {
      sprintf(errbuf,"Cannot allocate memory for \"dtype\".\n");
      H5Epush(__FILE__, "HE5_ZAdscaleattrinfoF2", __LINE__, H5E_RESOURCE, H5E_NOSPACE, errbuf);
      HE5_EHprint(errbuf, __FILE__, __LINE__);
      return(FAIL);
    }
  ...
  status = HE5_ZAdscaleattrinfo2(zaID, fieldname, attrname, dtype, count, size);
  if ( status == FAIL)
    {
      sprintf(errbuf, "Cannot retrieve attribute info for the %s dimension scale. \n", fieldname);
      H5Epush(__FILE__, "HE5_ZAdscaleattrinfoF2", __LINE__, H5E_ATTR, H5E_NOTFOUND, errbuf);
      HE5_EHprint(errbuf, __FILE__, __LINE__);
      free(count);
      free(dtype);
      return(FAIL);
...
|  BEGIN_PROLOG                                                               |
|                                                                             |
|  FUNCTION: HE5_ZAsetflddimlabel                                             |
|                                                                             |
|  DESCRIPTION: Sets label for a dimension in a field in a za.                |
|                                                                             |
|                                                                             |
|  Return Value    Type     Units     Description                             |
|  ============   ======  =========   =====================================   |
...
|  END_PROLOG                                                                 |
-----------------------------------------------------------------------------*/
int HE5_ZAsetflddimlabel(hid_t zaID, char *fieldname, char *dimname, char *dimlabel)
{
  hid_t          fid    = FAIL; 	       /* HDF-EOS file ID     */
  hid_t          gid    = FAIL;	               /* "HDFEOS" group ID   */
  hid_t          did;                          /* Field dataset id    */
  ...
  if(errbuf == NULL)
    {
      H5Epush(__FILE__, "HE5_ZAsetflddimlabel", __LINE__, H5E_FILE, H5E_NOSPACE, "Cannot allocate memory for error buffer.");
      HE5_EHprint("Error: Cannot allocate memory for error buffer, occured", __FILE__, __LINE__);
      return(FAIL);
    }
   ...
   ************************************************************************/
  status = HE5_ZAchkzaid(zaID, " HE5_ZAsetflddimlabel", &fid, &gid, &idx);
  if ( status == FAIL )
    {
      sprintf(errbuf, "Checking for za ID failed.\n");
      H5Epush(__FILE__, "HE5_ZAsetdimscale", __LINE__,  H5E_ARGS, H5E_BADRANGE, errbuf);
  ...
  labelsize = (int)HE5_ZAgetflddimlabel(zaID, fieldname, dimname, olddimlabel);
  if( labelsize == FAIL )
    {
      sprintf(errbuf, "Getting dimlabel failed.\n");
      H5Epush(__FILE__, "HE5_ZAsetflddimlabel", __LINE__,  H5E_ARGS, H5E_BADRANGE, errbuf);
      HE5_EHprint(errbuf, __FILE__, __LINE__);
      free(errbuf);
      return(FAIL);
    }
  ...
  /* for this field see if the dimlist contains dimname */
  status = HE5_ZAinfo(zaID, fieldname, &rank, tempdims,
			   &dtype, tempdimlist, NULL);

  if (status != SUCCEED)
    {
      sprintf(errbuf, "Field \"%s\" not found.\n", fieldname);
      H5Epush(__FILE__, "HE5_ZAsetflddimlabel", __LINE__, H5E_ARGS,
	      H5E_NOTFOUND, errbuf);
      HE5_EHprint(errbuf, __FILE__, __LINE__);
      free(errbuf);
      return(FAIL);
...
|  BEGIN_PROLOG                                                               |
|                                                                             |
|  FUNCTION: HE5_ZAgetflddimlabel                                             |
|                                                                             |
|  DESCRIPTION: Gets label for a dimension in a field in a za.                |
|                                                                             |
|                                                                             |
|  Return Value    Type     Units     Description                             |
|  ============   ======  =========   =====================================   |
...
|  END_PROLOG                                                                 |
-----------------------------------------------------------------------------*/
int HE5_ZAgetflddimlabel(hid_t zaID, char *fieldname, char *dimname, char *dimlabel)
{
  hid_t          fid    = FAIL; 	       /* HDF-EOS file ID     */
  hid_t          gid    = FAIL;	               /* "HDFEOS" group ID   */
  hid_t          did;                          /* Field dataset id    */
   ...
   * and za root Vgroup ID                                                *
   ************************************************************************/
  status = (int)HE5_ZAchkzaid(zaID, " HE5_ZAgetflddimlabel", &fid, &gid, &idx);
  if ( status == FAIL )
    {
      sprintf(errbuf, "Checking for za ID failed.\n");
      H5Epush(__FILE__, "HE5_ZAgetdimscale", __LINE__,  H5E_ARGS, H5E_BADRANGE, errbuf);
  ...
  if(found != 1) /* did not find fieldname */
    {
      sprintf(errbuf, "Fieldname \"%s\" does not exist.\n", fieldname);
      H5Epush(__FILE__, "HE5_ZAgetflddimlabel", __LINE__,  H5E_ARGS, H5E_BADRANGE, errbuf);
      HE5_EHprint(errbuf, __FILE__, __LINE__);
      free(errbuf);
      return(FAIL);
    }
  ...
  /* for this field see if the dimlist contains dimname */
  status = (int)HE5_ZAinfo(zaID, fieldname, &rank, tempdims,
			   &dtype, tempdimlist, NULL);

  if (status != SUCCEED)
    {
      sprintf(errbuf, "Field \"%s\" not found.\n", fieldname);
      H5Epush(__FILE__, "HE5_ZAgetflddimlabel", __LINE__, H5E_ARGS,
	      H5E_NOTFOUND, errbuf);
      HE5_EHprint(errbuf, __FILE__, __LINE__);
      free(errbuf);
      return(FAIL);
...
FCALLSCFUN5(INT, HE5_ZAatinfo2, HE5_ZAATTRINFO2, he5_zaattrinfo2, INT, STRING, PINT, PLONG, PLONG)
FCALLSCFUN5(INT, HE5_ZAgatinfo2, HE5_ZAGATTRINFO2, he5_zagattrinfo2, INT, STRING, PINT, PLONG, PLONG)
FCALLSCFUN6(INT, HE5_ZAlatinfo2, HE5_ZALATTRINFO2, he5_zalattrinfo2, INT, STRING, STRING, PINT, PLONG, PLONG)
...
FCALLSCFUN6(INT, HE5_ZAdefdimscale2F, HE5_ZADEFDIMSCALE2, he5_zadefdimscale2,INT,STRING,STRING,LONG,INT,PVOID)
FCALLSCFUN6(INT, HE5_ZAsetdimscaleF, HE5_ZASETDIMSCALE, he5_zasetdimscale,INT,STRING,STRING,LONG,INT,PVOID)
FCALLSCFUN7(INT, HE5_ZAsetdimscale2F, HE5_ZASETDIMSCALE2, he5_zasetdimscale2,INT,STRING,STRING,STRING,LONG,INT,PVOID)
...
FCALLSCFUN6(INT, HE5_ZAdscaleattrinfoF2, HE5_ZADSCALEATTRINFO2, he5_zadscaleattrinfo2,INT,STRING,STRING,PINT,PLONG,PLONG)
FCALLSCFUN4(INT, HE5_ZAinqdscaleattrsF, HE5_ZAINQDSCALATTRS, he5_zainqdscaleattrs,INT,STRING,STRING,PLONG)
FCALLSCFUN4(INT, HE5_ZAsetflddimlabel, HE5_ZASETFLDDIMLABEL, he5_zasetflddimlabel, INT, STRING, STRING, STRING);
FCALLSCFUN4(INT, HE5_ZAgetflddimlabel, HE5_ZAGETFLDDIMLABEL, he5_zagetflddimlabel, INT, STRING, STRING, STRING);
