...
#ifndef MAX_ORDER
#define MAX_ORDER          65535 /* for writing metadata with HE5_EHwritemeta
				    compatible with TOOLKIT Coremetadata
				    writing */
#endif
...
/*int HDFEOS5ver_flg ; *//* will be set to 1 if the HDF-EOS5 version
			    is NOT later than 5.1.7, otherwise to 0 */
...
typedef struct
{
  ...
  int               DYNAMIC_flag; /*  Flage for checking for the compatibality
				      ...
				      HDF-EOS5 */
  int               HE5_STATIC_BLKSIZE; /* will be used, after modifications
					   in EHopen, for buffer size memory
					   allocatins of StructMetadata.0 for
					   static method */
...
HE5_EHdtype2mtype(hid_t dtype)
{
  ...
  if (nativetype == FAIL)
	...
	if (H5Tequal(typelists[i], nativetype) ) {
	  mtype = typelists[i];
	  break;
	}
      }
...
HE5_EHopen(const char *filename, uintn flags,  hid_t access_id)
{
  ...
  if (nfileopen < HE5_NEOSHDF)
      ...
      else if (flags == H5F_ACC_TRUNC)
	{
	  ...
	  /* H5Eset_auto(NULL,NULL); */
	  /*
	     To add very big attributes to HDF5 objects we need to create HDF5 file
	     as below, instead of just using H5Fcreate()
	  */

	  fapid = H5Pcreate(H5P_FILE_ACCESS);
	  opstatus = H5Pset_libver_bounds(fapid, H5F_LIBVER_LATEST, H5F_LIBVER_LATEST);
	  HDFfid = H5Fcreate(filename,flags,H5P_DEFAULT,fapid);
	  opstatus = H5Pclose(fapid);
	  if(HDFfid < 0)
	    {
...
HE5_EHparsestr(const char *instring, char delim, char *pntr[], size_t len[])
{
  ...
  delimitor = strchr((char *)instring, delim);
...
|  BEGIN_PROLOG                                                               |
|                                                                             |
|  FUNCTION: HE5_EHstrwithin_index                                            |
|                                                                             |
|  DESCRIPTION: Simulate HE5_EHstrwithin but return all the indexes of the    |
|                found string.                                                |
|                                                                             |
|  The return value returns how many elements in the target that has search   |
|  when index_found is passed as NULL.                                        |
|  This then can be used to allocate memory to hold the index_found when      |
|  passing this function second time to obtain all the index of search. See   |
|  SWapi.c for the use of this function.                                      |
|                                                                             |
|  END_PROLOG                                                                 |
-----------------------------------------------------------------------------*/
long
HE5_EHstrwithin_index(char *target, char *search, char delim,int* index_found)
{
  ...
  int          num_index_found = 0;
  int*         temp_index_found = NULL;
  int*         temp_index_ptr = NULL;
  int          i = 0;
  ...
  temp_index_found = (int *)calloc(nentries,sizeof(int));
	  ...
	  if (strcmp(target, buffer) == 0)
        {
             if(found != 0)
		  found = 1;
             temp_index_found[num_index_found] = indx;
             num_index_found++;
        }
    }
  ...
  if (found == 0)
	indx = FAIL;
  temp_index_ptr = index_found;
  if(index_found) {
     for(i = 0; i <num_index_found;i++) {
        *temp_index_ptr = temp_index_found[i];
        temp_index_ptr++;
     }
  }
...
HE5_EHattrinfo( hid_t grpID, const char *attrname, hid_t *datatype, hsize_t *count)
{
  ...
  status = HE5_EHattrinfo2( grpID, attrname, datatype, count, bufsize);
  if(status == FAIL)
    {
      sprintf(errbuf,"Cannot retrieve information about Attribute \"%s\" \n", attrname);
      H5Epush(__FILE__, "HE5_EHattrinfo", __LINE__, H5E_FUNC, H5E_CANTINIT, errbuf);
      HE5_EHprint(errbuf, __FILE__, __LINE__);
      return(status);
    }
  return(status);
...
|  BEGIN_PROLOG                                                               |
|                                                                             |
|  FUNCTION: HE5_EHattrinfo2                                                  |
|                                                                             |
|  DESCRIPTION: Returns numbertype, count and buffer size of given HDF-EOS    |
|               attribute                                                     |
|                                                                             |
|                                                                             |
|  Return Value    Type     Units     Description                             |
...
-----------------------------------------------------------------------------*/
herr_t
HE5_EHattrinfo2( hid_t grpID, const char *attrname, hid_t *datatype, hsize_t *count, hsize_t *size)
{
  herr_t      status     = FAIL;         /* Return status variable  */
...
-----------------------------------------------------------------------------*/
hid_t
HE5_EHconvdatatype(hid_t fdatatype)
{
  ...
  /* -------------------------------------------------- */
  if(fdatatype == HE5T_NATIVE_INT)
    cdatatype = H5T_NATIVE_INT;
  else if(fdatatype == HE5T_NATIVE_UINT)
    cdatatype = H5T_NATIVE_UINT;
  else if(fdatatype == HE5T_NATIVE_SHORT)
    cdatatype = H5T_NATIVE_SHORT;
  else if(fdatatype == HE5T_NATIVE_USHORT)
    cdatatype = H5T_NATIVE_USHORT;
  else if(fdatatype == HE5T_NATIVE_SCHAR)
    cdatatype = H5T_NATIVE_SCHAR;
  else if(fdatatype == HE5T_NATIVE_UCHAR)
    cdatatype = H5T_NATIVE_UCHAR;
  else if(fdatatype == HE5T_NATIVE_LONG)
    cdatatype = H5T_NATIVE_LONG;
  else if(fdatatype == HE5T_NATIVE_ULONG)
    cdatatype = H5T_NATIVE_ULONG;
  else if(fdatatype == HE5T_NATIVE_LLONG)
    cdatatype = H5T_NATIVE_LLONG;
  else if(fdatatype == HE5T_NATIVE_ULLONG)
    cdatatype = H5T_NATIVE_ULLONG;
  else if(fdatatype == HE5T_NATIVE_FLOAT)
    cdatatype = H5T_NATIVE_FLOAT;
  else if(fdatatype == HE5T_NATIVE_DOUBLE)
    cdatatype = H5T_NATIVE_DOUBLE;
  else if(fdatatype == HE5T_NATIVE_LDOUBLE)
    cdatatype = H5T_NATIVE_LDOUBLE;
  else if(fdatatype == HE5T_NATIVE_INT8)
    {
      cdatatype = H5Tcopy(H5T_NATIVE_INT);
      H5Tset_size(cdatatype, 1);
    }
  else if(fdatatype == HE5T_NATIVE_UINT8)
    {
      cdatatype = H5Tcopy(H5T_NATIVE_UINT);
      H5Tset_size(cdatatype, 1);
    }
  else if(fdatatype == HE5T_NATIVE_INT16)
    {
      cdatatype = H5Tcopy(H5T_NATIVE_INT);
      H5Tset_size(cdatatype, 2);
    }
  else if(fdatatype == HE5T_NATIVE_UINT16)
    {
      cdatatype = H5Tcopy(H5T_NATIVE_UINT);
      H5Tset_size(cdatatype, 2);
    }
  else if(fdatatype == HE5T_NATIVE_INT32)
    {
      cdatatype = H5Tcopy(H5T_NATIVE_INT);
      H5Tset_size(cdatatype, 4);
    }
  else if(fdatatype == HE5T_NATIVE_UINT32)
    {
      cdatatype = H5Tcopy(H5T_NATIVE_UINT);
      H5Tset_size(cdatatype, 4);
    }
  else if(fdatatype == HE5T_NATIVE_INT64)
    {
      cdatatype = H5Tcopy(H5T_NATIVE_INT);
      H5Tset_size(cdatatype, 8);
    }
  else if(fdatatype == HE5T_NATIVE_UINT64)
    {
      cdatatype = H5Tcopy(H5T_NATIVE_UINT);
      H5Tset_size(cdatatype, 8);
    }
  else if(fdatatype == HE5T_NATIVE_B8)
    cdatatype = H5T_NATIVE_B8;
  else if(fdatatype == HE5T_NATIVE_B16)
    cdatatype = H5T_NATIVE_B16;
  else if(fdatatype == HE5T_NATIVE_B32)
    cdatatype = H5T_NATIVE_B32;
  else if(fdatatype == HE5T_NATIVE_B64)
    cdatatype = H5T_NATIVE_B64;
  else if(fdatatype == HE5T_NATIVE_HSIZE)
    cdatatype = H5T_NATIVE_HSIZE;
  else if(fdatatype == HE5T_NATIVE_HERR)
    cdatatype = H5T_NATIVE_HERR;
  else if(fdatatype == HE5T_NATIVE_HBOOL)
    cdatatype = H5T_NATIVE_HBOOL;
  else if(fdatatype == HE5T_STD_I8BE || fdatatype == HE5T_STD_I8LE)
    cdatatype = H5T_NATIVE_SCHAR;
  else if(fdatatype == HE5T_STD_I16BE || fdatatype == HE5T_STD_I16LE)
    cdatatype = H5T_NATIVE_SHORT;
  else if(fdatatype == HE5T_STD_I32BE || fdatatype == HE5T_STD_I32LE)
    cdatatype = H5T_NATIVE_INT;
  else if(fdatatype == HE5T_STD_I64BE|| fdatatype == HE5T_STD_I64LE)
    cdatatype = H5T_NATIVE_LONG;
  else if(fdatatype == HE5T_STD_U8BE || fdatatype == HE5T_STD_U8LE)
    cdatatype = H5T_NATIVE_UCHAR;
  else if(fdatatype == HE5T_STD_U16BE || fdatatype == HE5T_STD_U16LE)
    cdatatype = H5T_NATIVE_USHORT;
  else if(fdatatype == HE5T_STD_U32BE || fdatatype == HE5T_STD_U32LE)
    cdatatype = H5T_NATIVE_UINT;
  else if(fdatatype == HE5T_STD_U64BE || fdatatype == HE5T_STD_U64LE)
    cdatatype = H5T_NATIVE_ULONG;
  else if(fdatatype == HE5T_STD_B8BE)
    cdatatype = H5T_STD_B8BE;
  else if(fdatatype == HE5T_STD_B8LE)
    cdatatype = H5T_STD_B8LE;
  else if(fdatatype == HE5T_STD_B16BE)
    cdatatype = H5T_STD_B16BE;
  else if(fdatatype == HE5T_STD_B16LE)
    cdatatype = H5T_STD_B16LE;
  else if(fdatatype == HE5T_STD_B32BE)
    cdatatype = H5T_STD_B32BE;
  else if(fdatatype == HE5T_STD_B32LE)
    cdatatype = H5T_STD_B32LE;
  else if(fdatatype == HE5T_STD_B64BE)
    cdatatype = H5T_STD_B64BE;
  else if(fdatatype == HE5T_STD_B64LE)
    cdatatype = H5T_STD_B64LE;
  else if(fdatatype == HE5T_IEEE_F32BE || fdatatype == HE5T_IEEE_F32LE)
    cdatatype = H5T_NATIVE_FLOAT;
  else if(fdatatype == HE5T_IEEE_F64BE || fdatatype == HE5T_IEEE_F64LE)
    cdatatype = H5T_NATIVE_DOUBLE;
  else if(fdatatype == HE5T_NATIVE_CHAR)
    cdatatype = H5T_NATIVE_CHAR;
  else if(fdatatype == HE5T_CHARSTRING)
    cdatatype = H5T_C_S1;
...
|                                                                             |
|  DESCRIPTION: Wrapper around HE5_EHglbattrinfo2 that drops attribute buffer |
|                size. Gives to user only count as before                     |
|                                                                             |
|  Return Value    Type     Units     Description                             |
|  ============   ======  =========   =====================================   |
|  status         herr_t              return status (0) SUCCEED, (-1) FAIL    |
...
HE5_EHglbattrinfo(hid_t fid, const char *attrname, hid_t *ntype, hsize_t *count)
{
  ...
  status = HE5_EHglbattrinfo2(fid, attrname, ntype, count, bufsize);
  if(status == FAIL)
    {
...
|  ntype          hid_t               data type ID                            |
|  count          hsize_t             Number of attribute elements            |
|  size           hsize_t *           buffer size of attribute elements       |
|                                                                             |
|                                                                             |
|  OUTPUTS:                                                                   |
|             None                                                            |
...
-----------------------------------------------------------------------------*/
herr_t
HE5_EHglbattrinfo2(hid_t fid, const char *attrname, hid_t *ntype, hsize_t *count, hsize_t *size)
{
  herr_t     status          = FAIL;     /* Return status variable     */
  ...
  if (status == SUCCEED)
      ...
      if(FileAttrGrpID == FAIL)
	{
      ...
      status = HE5_EHattrinfo2(FileAttrGrpID,attrname,ntype,count,size);
      if(status == FAIL)
	{
...
HE5_EHupdatemeta_Dynamic(hid_t fid, const char *structname, char *structcode, long metacode, char *metastr, hsize_t metadata[])
{
  ...
  int            location_updated_eos5obj_name = 0;
  int            location_updated_dimension_name = 0;
  int            location_updated_dimension_size = 0;
  int            utlstr2len = 0;
  int            newbuflen = 0;
  int            temp_sizestrlen = 0;
  int            metadatalen_left_sindex = 0;

  char* temp_smetadata_eos5obj_section = NULL;
  char* temp_smetadata_dimname_section = NULL;
  char* temp_smetadata_dimsize_section = NULL;
  char* temp_sizestr="Size=";
  char* temp_sizedata=NULL;
  ...
  switch ((int)abs((int)metacode))
    ...
    case 0:
      ...
      if(
	 ((strcmp(metastr, "XDim") == 0) || (strcmp(metastr, "YDim") == 0)))
	{
	  /* XDim and YDim assumed to be non-appendable
	     (also cannot have Unlimited dimensions)
	  */
	  return 0;
	}
      else
	{
	  sprintf(utlstr, "%s%li%s%s%s%li%s%li%s", "\t\t\tOBJECT=Dimension_",count,"\n\t\t\t\tDimensionName=\"",&metastr[0],"\"\n\t\t\t\tSize=", (long)metadata[0],"\n\t\t\tEND_OBJECT=Dimension_", count, "\n");
	  /* // This section is only used to update the dimension size of unlimited dimension. */
	  {
	    newbuf = (char *) calloc(HE5_DYNAMIC_BLKSIZE * (nmeta), sizeof(char));
	    if(newbuf==NULL)
	      {
	    ...
	    temp_smetadata_eos5obj_section =
	      strstr((const char*)newbuf,utlstr);

	    /* do not let NULL pointer to cause core dump */
	    if(temp_smetadata_eos5obj_section == NULL) return 0;

	    location_updated_eos5obj_name =
	      strlen(newbuf)-strlen(temp_smetadata_eos5obj_section);

	    temp_smetadata_dimname_section =
	      strstr((const char*)temp_smetadata_eos5obj_section,metastr);

	    location_updated_dimension_name =
	      strlen(temp_smetadata_eos5obj_section)-strlen(temp_smetadata_dimname_section);

	    /* do not let NULL pointer to cause core dump */
	    if(temp_smetadata_dimname_section == NULL) return 0;

	    temp_smetadata_dimsize_section =
	      strstr((const char*)temp_smetadata_dimname_section,temp_sizestr);

	    location_updated_dimension_size =
	      strlen(temp_smetadata_dimname_section)-strlen(temp_smetadata_dimsize_section);

	    temp_sizedata =
	      strchr(temp_smetadata_dimsize_section,'\n');
	    ...
	    /*// The first untouched part */
	    ii = location_updated_eos5obj_name + location_updated_dimension_name + location_updated_dimension_size+temp_sizestrlen;

	    /*// The changed part (from SIZE= ?? to SIZE=**) */
	    if((utlstr2len+ii+1) <strlen(newbuf)) {

	      for(i = 0;i<utlstr2len;i++) {
		newbuf[ii+i] = utlstr2[i];
	      }

	      /* // strlen(temp_smetadata_dimsize_section)-strlen(temp_sizedata) gives the length
	      // SIZE=?? ,we then need to add the length of "SIZE="(temp_sizestrlen) to get the
	      // final untouched part of the string. */
	      metadatalen_left_sindex =
		ii+((strlen(temp_smetadata_dimsize_section)-strlen(temp_sizedata))-temp_sizestrlen);

	      /*// The second part(moves to the end of the original second part) */
	      for(i=metadatalen_left_sindex;i<newbuflen;i++) {

		newbuf[ii+utlstr2len-metadatalen_left_sindex+i] = prevmetaptr[i];
...
/* get number of metadatas for StructMetadata.        *
 * The call HE5_GDinqmetadatas gets number of all     *
 * meatadata datasets (jn addition to StructMetadata  *
 * in HDFEOS INFORMATION group.                       */
...
int HE5_szip_can_encode(void )
{
  ...
  herr_t       status;
  unsigned int filter_config_flags = 0; /* false */
...
|  BEGIN_PROLOG                                                               |
|                                                                             |
|  FUNCTION: HE5_EHwritemeta                                                  |
|                                                                             |
|  DESCRIPTION: Writes metadata to "HDFEOS INFORMATION"                       |
|                                                                             |
|                                                                             |
|  Return Value    Type     Units     Description                             |
|  ============   ======  =========   =====================================   |
...
|                                                                             |
|    fid          hid_t   None     HDF-EOS file ID                            |
|    MetadataName char*   None     Name of the metadata (such as Coremetadata)|
|    metabuff     char*   None     Buffer containing metadata information     |
|                                                                             |
|  OUTPUTS:                                                                   |
|                                                                             |
|    None                                                                     |
|                                                                             |
|  NOTES:   There is memory allocation in cuncks of MAX_ORDER = 65535         ]
|           allocated dynamically.                                            |
|           The chuncks will be written to datasets "MetadataName",           |
|           "MetadataName.1", "MetadataName.2", etc.                          |
|                                                                             |
|   Date       Programmer   Description                                       |
|  ======     ============  ===============================================   |
|  12/1/2015   Abe Taaheri  Initial version                                   |
|                                                                             |
|  END_PROLOG                                                                 |
-----------------------------------------------------------------------------*/
herr_t
HE5_EHwritemeta(hid_t fid, const char *MetadataName, char *metabuff)
{
  herr_t   status = FAIL;
  hid_t    HDFfid    = FAIL;/* HDF file ID  */
  int      numAttr, buffsize;
  char    *chunkBuffer = NULL, *chunkBuffer_temp = NULL;
  char    *newAttrName = NULL;
  int      hdfRetVal = SUCCEED;
  hid_t           ggid;
  hid_t           sid2;
  hid_t           datid;
  size_t          size;
  ...
  if(errbuf == NULL)
    {
	  H5Epush(__FILE__, "HE5_EHwritemeta", __LINE__, H5E_FILE, H5E_NOSPACE,
  ...
  numAttr = buffsize/MAX_ORDER + 1; /* number of MAX_ORDER chunks in metadata */

  chunkBuffer = (char *)malloc((int)(MAX_ORDER + 1)*sizeof(char));
  if(chunkBuffer == NULL)
    {
      H5Epush(__FILE__, "HE5_EHwritemeta", __LINE__, H5E_FILE, H5E_NOSPACE,
	      "Cannot allocate memory for error buffer. \n");
  ...
  if( ggid < 0 )
	...
	{
	  sprintf(errbuf, "Cannot create \"HDFEOS INFORMATION\" group.");
	  H5Epush(__FILE__, "HE5_EHwritemeta", __LINE__, H5E_OHDR, H5E_CANTINIT, errbuf);
	  HE5_EHprint(errbuf, __FILE__, __LINE__);
	  free(errbuf);
	  free(chunkBuffer);
	  free(newAttrName);
  ...
  for (i = 0; i < numAttr; i++)
    {
      remainingbuf = buffsize - i*MAX_ORDER;
      if(remainingbuf > MAX_ORDER)
	{
	  memmove(chunkBuffer, metabuff + (i * MAX_ORDER) , MAX_ORDER);
	  chunkBuffer[MAX_ORDER]= '\0';
	}
      else
	{
	  memmove(chunkBuffer, metabuff + (i * MAX_ORDER) , remainingbuf);
	  chunkBuffer[remainingbuf]= '\0';
	}
      ...
      else
	{
      ...
      /*
	 -------------------------------------------------------
	 |Create dataset  "MetadataName, MetadataName.0, etc   |
      ...
      if (sid2 == -1)
	{
	  ...
	  H5Epush(__FILE__, "HE5_EHwritemeta", __LINE__, H5E_DATASPACE, H5E_CANTCREATE, errbuf);
	  hdfRetVal = FAIL;
	}
      ...
      if(hdfRetVal == SUCCEED)
	{
	  ...
	  if (status == -1)
	    {
	      sprintf(errbuf, "Cannot set the total size for atomic datatype. \n");
	      H5Epush(__FILE__, "HE5_EHwritemeta", __LINE__, H5E_DATATYPE, H5E_CANTCREATE, errbuf);
	      hdfRetVal = FAIL;
	    }
	}
      ...
      if(hdfRetVal == SUCCEED)
	{
	  ...
	  if(datid == -1)
		...
		{
		  sprintf(errbuf, "Cannot create %s dataset. \n", newAttrName);
		  H5Epush(__FILE__, "HE5_EHwritemeta", __LINE__, H5E_DATASET, H5E_CANTCREATE, errbuf);
		  hdfRetVal = FAIL;
		}
	    }
	}
      ...
      */
      ...
      chunkBuffer_temp = (char *) malloc((MAX_ORDER + 1)*sizeof(char));
      /* initialize memory */
      for(ism = 0; ism < (MAX_ORDER + 1); ism++)
	{
	  chunkBuffer_temp[ism] = '\0';
	}
      strcpy(chunkBuffer_temp, chunkBuffer);
      status = H5Dwrite(datid,atype,H5S_ALL,H5S_ALL,H5P_DEFAULT,chunkBuffer_temp);
      free(chunkBuffer_temp);
      chunkBuffer_temp = (char *) NULL;

      if( status == FAIL)
	{
	  sprintf(errbuf, "Cannot write in data to the %s dataset.",newAttrName);
	  H5Epush(__FILE__, "HE5_EHwritemeta", __LINE__, H5E_DATASET, H5E_WRITEERROR, errbuf);
	  hdfRetVal = FAIL;
	}
      ...
      if(hdfRetVal == SUCCEED)
	{
	  ...
	  if (status == FAIL)
	    {
	      sprintf(errbuf,"Cannot release the dataspace ID.");
	      H5Epush(__FILE__, "HE5_EHwritemeta", __LINE__, H5E_DATASPACE, H5E_CLOSEERROR, errbuf);
	      hdfRetVal = FAIL;
	    }
	}
      ...
      if(hdfRetVal == SUCCEED)
	{
	  ...
	  if (status == FAIL)
	    {
	      sprintf(errbuf, "Cannot release the dataset ID.");
	      H5Epush(__FILE__, "HE5_EHwritemeta", __LINE__, H5E_DATASET, H5E_CLOSEERROR, errbuf);
	      hdfRetVal = FAIL;
	    }
	}
  ...
  if (status == FAIL)
    {
      sprintf(errbuf, "Cannot release the \"HDFEOS INFORMATION\" group ID. \n");
      H5Epush(__FILE__, "HE5_EHwritemeta", __LINE__, H5E_OHDR, H5E_CLOSEERROR, errbuf);
      HE5_EHprint(errbuf, __FILE__, __LINE__);
      free(errbuf);
      free(chunkBuffer);
      free(newAttrName);
...
|  OUTPUTS:                                                                   |
|    metabuff     char*    None     Buffer containing metadata information    |
|    bufferSize   long     None     size of Buffer containing metadata        |
|                                                                             |
|                                                                             |
|  NOTES:   There is memory allocation in cuncks of MAX_ORDER = 65535         ]
|           allocated dynamically.                                            |
|           The chuncks will be read from datasets "MetadataName",            |
|           "MetadataName.1", "MetadataName.2", etc.                          |
|                                                                             |
|   Date         Programmer    Description                                    |
|  ==========    ============  ============================================   |
|  03/15/2017    Abe Taaheri   Initial version                                |
|                                                                             |
|  END_PROLOG                                                                 |
-----------------------------------------------------------------------------*/
herr_t
HE5_EHreadmeta(hid_t fid, const char *MetadataName, char *metabufout, long *bufferSize)
{
  herr_t   status = FAIL;
  hid_t    HDFfid = FAIL;           /* HDF file ID  */
  char    *newAttrName = NULL;
 ...
 /* Allocate memory for error message buffer */
  ...
  if(errbuf == NULL)
    {
      H5Epush(__FILE__, "HE5_EHreadmeta", __LINE__, H5E_FILE, H5E_NOSPACE,
	      "Cannot allocate memory for error buffer. \n");
  ...
  if(newAttrName == NULL)
    {
      H5Epush(__FILE__, "HE5_EHreadmeta", __LINE__, H5E_FILE, H5E_NOSPACE,
	      "Cannot allocate memory for error buffer. \n");
      HE5_EHprint("Error: Cannot allocate memory for newAttrName buffer.", __FILE__, __LINE__);
      free(errbuf);
      return(FAIL);
    }
  ...
  if( ggid < 0 )
    {
      sprintf(errbuf, "Cannot open \"HDFEOS INFORMATION\" group.");
      H5Epush(__FILE__, "HE5_EHreadmeta", __LINE__, H5E_OHDR, H5E_CANTINIT, errbuf);
      HE5_EHprint(errbuf, __FILE__, __LINE__);
      free(errbuf);
      free(newAttrName);
      return(FAIL);
  ...
  /* Determine number of metadata "sections" */
  /* -------------------------------------------------- */
  nmeta = 0;
   /* status= HE5_EHgetnmeta(ggid, &nmeta); */
  nmeta = HE5_EHinqmetadatas(fid, (char *)NULL, &strbufsize);
  if(nmeta == 0 || nmeta < 0)
    {
      status = FAIL;
    }
  ...
  if(status == FAIL)
    {
      ...
      H5Epush(__FILE__, "HE5_EHreadmeta", __LINE__, H5E_OHDR, H5E_NOTFOUND, errbuf);
      HE5_EHprint(errbuf, __FILE__, __LINE__);
      free(errbuf);
      free(newAttrName);
      return(FAIL);
    ...
    /* Allocate space for metadata (in units of  bytes) */
    /* ----------------------------------------------------- */
    metabuf = (char *) calloc((int)(MAX_ORDER + 1) * nmeta, sizeof(char ));
    if(metabuf == NULL)
    {
      sprintf(errbuf, "Cannot allocate memory for the metadata buffer. \n");
      H5Epush(__FILE__, "HE5_EHinsertmeta_Dynamic", __LINE__, H5E_RESOURCE, H5E_NOSPACE, errbuf);
    ...
    /* initialize memory */
   for(ism = 0; ism < (MAX_ORDER + 1) * nmeta; ism++)
      {
	metabuf[ism] = '\0';
      }
  /* emplty MetadataName to add data later */
  strcpy(metabuf, "");

  /* Read  metadata from all metadata datasets */
  /* --------------------------------------------------------- */
  for (ism = 0; ism < (nmeta+ 1); ism++)
    {
       if(ism == 0)
	{
	  sprintf(newAttrName, "%s", MetadataName);
	...
	meta_id[ism] = H5Dopen(ggid, newAttrName);
	metalen = strlen(metabuf);
	...
	if( meta_id[ism] == FAIL)
	  {
	    ...
	    /* look for next metadata dataset */
	    continue;
	  }
	...
	if( atype[ism] == FAIL)
	  {
	    status = FAIL;
	    sprintf(errbuf,"Cannot get the dataset datatype. \n");
	    H5Epush(__FILE__, "HE5_EHreadmeta", __LINE__, H5E_DATATYPE, H5E_NOTFOUND, errbuf);
	    HE5_EHprint(errbuf, __FILE__, __LINE__);
	    free(errbuf);
	    free(metabuf);
	    status = H5Dclose(meta_id[ism]);
	...
	if( aspace[ism] == FAIL )
	  {
	    status = FAIL;
	    sprintf(errbuf,"Cannot get the dataset dataspace. \n");
	    H5Epush(__FILE__, "HE5_EHreadmeta", __LINE__, H5E_DATASPACE, H5E_NOTFOUND, errbuf);
	    HE5_EHprint(errbuf, __FILE__, __LINE__);
	    free(errbuf);
	    free(metabuf);
	    status = H5Tclose(atype[ism]);
	...
	/*
	 *-------------------------------------------------------------*
	 *   Allocated memory for the output array, now Read attribute *
	 *-------------------------------------------------------------*
	 */
	...
	if( status == FAIL)
	  {
	    ...
	    H5Epush(__FILE__, "HE5_EHreadmeta", __LINE__, H5E_DATASET, H5E_READERROR, errbuf);
	    HE5_EHprint(errbuf, __FILE__, __LINE__);
	    free(errbuf);
	    free(metabuf);
	    return(status);
  ...
  */
  ...
  /* close  metadata for all metadata datasets */
  /* --------------------------------------------------------- */
  for (ism = 0; ism < (nmeta+ 1); ism++)
    {
      if(aspace[ism] != FAIL)
	{
	  ...
	  if (status == FAIL)
	    {
	      sprintf(errbuf,"Cannot release the dataspace ID.");
	      H5Epush(__FILE__, "HE5_EHreadmeta", __LINE__, H5E_DATASPACE, H5E_CLOSEERROR, errbuf);
	      HE5_EHprint(errbuf, __FILE__, __LINE__);
	      hdfRetVal = FAIL;
	    }
	}
       ...
       if(atype[ism] != FAIL)
	{
	  ...
	  if( status == FAIL)
	    {
	      sprintf(errbuf, "Cannot release the data type ID.\n");
	      H5Epush(__FILE__, "HE5_EHreadmeta", __LINE__, H5E_DATATYPE, H5E_CLOSEERROR, errbuf);
	      HE5_EHprint(errbuf, __FILE__, __LINE__);
	      hdfRetVal = FAIL;
	    }
	}
       ...
       if(meta_id[ism]  != FAIL)
	{
	  ...
	  if (status == FAIL)
	    {
	      sprintf(errbuf, "Cannot release the dataset ID.");
	      H5Epush(__FILE__, "HE5_EHreadmeta", __LINE__, H5E_DATASET, H5E_CLOSEERROR, errbuf);
	      HE5_EHprint(errbuf, __FILE__, __LINE__);
	      hdfRetVal = FAIL;
	    }
	}
  ...
  if (status == FAIL)
    {
      sprintf(errbuf, "Cannot release the \"HDFEOS INFORMATION\" group ID. \n");
      H5Epush(__FILE__, "HE5_EHreadmeta", __LINE__, H5E_OHDR, H5E_CLOSEERROR, errbuf);
      HE5_EHprint(errbuf, __FILE__, __LINE__);
      free(errbuf);
      free(newAttrName);
      return(FAIL);
...
|  BEGIN_PROLOG                                                               |
|                                                                             |
|  FUNCTION: HE5_EHinqmetadatas                                               |
|                                                                             |
|  DESCRIPTION:  Gets the string containing metadata datasets in              |
|                HDFEOS INFORMATION group.                                    |
|                                                                             |
|                                                                             |
|  Return Value    Type     Units     Description                             |
|  ============   ======  =========   =====================================   |
|  nmeta          long                number of metadatas  (-1 if fails)      |
|                                                                             |
|  INPUTS:                                                                    |
|  fileID         hid_t   None        HDF-EOS5 type file  ID                  |
|                                                                             |
|  OUTPUTS:                                                                   |
|  metadatanames  char*               metadata name(s)   (comma-separated)    |
|  strbufsize     long                String length of metadatas (in bytes)   |
|                                                                             |
|  NOTES:                                                                     |
|         Returns only the first metadata name for the metadata that is       |
|         broken into sevral pices, such as myMeta, myMeta.01, myMeta.02, etc |
|                                                                             |
|  None                                                                       |
|                                                                             |
|   Date       Programmer    Description                                      |
|  ======      ============  ==============================================   |
|  03/16/2017  Abe Taaheri   Added the api                                    |
|                                                                             |
|  END_PROLOG                                                                 |
-----------------------------------------------------------------------------*/
long
HE5_EHinqmetadatas(hid_t fid, char *metadatanames, long *strbufsize)
{
  herr_t          status   = FAIL;/* routine return status variable  */
  hid_t           HDFfid = FAIL;           /* HDF file ID  */
  hid_t           gid      = FAIL;/* "HDFEOS" group ID               */
  hid_t           grpID      = FAIL;
  uintn           access;
  long            nmeta    =  0;  /* Number of attributes            */
  ...
  /* Determine number of metadata datasets */
  /* --------------------------------------*/
  nmeta = 0;
  status = HE5_EHfilename(fid, filename);
  infile = H5Fopen(filename, H5F_ACC_RDONLY, H5P_DEFAULT);

  strcpy(hdfeosgroup, "/HDFEOS INFORMATION");
  HIgid = H5Gopen1(infile,hdfeosgroup);

  /*
   *  Get all the members of the groups, one at a time.
   */

  err = H5Gget_num_objs(HIgid, &nobj);
  if(nobj < 1)
    {
      return(nmeta);
    }
  ...
  *strbufsize = 0;
  /* If metadata names are desired then "clear" name buffer */
  /* ------------------------------------------------------ */
  ...
  for (i = 0; i < nobjs; i++) {
     ...
     *   what type of object it is. ignore all but datasets.
     */
    strcpy(memb_name, "");
      len = H5Gget_objname_by_idx(HIgid, (hsize_t)i,
				  memb_name, (size_t)MAX_NAME );

      otype =  H5Gget_objtype_by_idx(HIgid, (size_t)i );
      /*
	...
	/*
	printf(" SYM_LINK:\n");
	do_link(ingid,memb_name);
	*/
	break;
      case H5G_GROUP:
	/*
	printf(" GROUP:\n");
	outgrpid = H5Gopen1(outgid, memb_name);

	if(outgrpid < 0) continue; // Group is not in the output. Bail out.

	ingrpid = H5Gopen1(ingid,memb_name);
	transfer_all_groups_attr_H5(ingrpid, outgrpid, outfile_flag);
	H5Gclose(ingrpid);
	H5Gclose(outgrpid);
	*/
	break;
      case H5G_DATASET:
	datasetid     = H5Dopen1(HIgid, memb_name);
	datatype      = H5Dget_type(datasetid);
	classid      = H5Tget_class(datatype);

	if(strcmp(memb_name,"StructMetadata.0")==0)
	  {
	    strcpy(memb_name, "StructMetadata");
	    len =(long)strlen(memb_name);
	  }
	*strbufsize = *strbufsize  + (int)len;
	    /* Get metadatanames (if desired) */
	    /* --------------------------- */
	if(metadatanames != NULL)
	  {
	    if(nmeta > 0)
	      {
		if((strstr(memb_name, ".0" ) != NULL ||
		    strstr(memb_name, ".1" ) != NULL ||
		    strstr(memb_name, ".2" ) != NULL ||
		    strstr(memb_name, ".3" ) != NULL ||
		    strstr(memb_name, ".4" ) != NULL ||
		    strstr(memb_name, ".5" ) != NULL ||
		    strstr(memb_name, ".6" ) != NULL ||
		    strstr(memb_name, ".7" ) != NULL ||
		    strstr(memb_name, ".8" ) != NULL ||
		    strstr(memb_name, ".9" ) != NULL ))
		  {
		    continue;
		  }
		else
...
FCALLSCFUN3(INT, HE5_EHwritemeta, HE5_EHWRITEMETA, he5_ehwritemeta, INT, STRING, STRING)
FCALLSCFUN4(INT, HE5_EHreadmeta, HE5_EHREADMETA, he5_ehreadmeta, INT, STRING, STRING, PLONG)
#ifdef H5_HAVE_FILTER_SZIP
